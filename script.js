...
labelTotalWaste1.dx = 100;
labelTotalWaste1.dy = -150;

labelTotalWaste1.text = usTotalWaste[2003];
...
categoryAxisRenderer.fontSize = 25;
categoryAxisRenderer.fontWeight = "light";
...
scaleRangeSlider.events.on("rangechanged", function () {
    ...
    // valueAxis.max = 100000 * Math.pow(10,start*4);
})
    ...
    series.adapter.add("fill", function(fill, target) {
      for (var continent in waste){
        ...
        if (target.dataItem && (target.dataItem.valueY < 0)) {
          return colorSet.getIndex(index * 6);
        }
      else {
        return fill;
      }
